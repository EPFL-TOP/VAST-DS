...
plt.subplot(211)
plt.hist(n_total, bins=20, alpha=0.6, label="Total somites", density=True)
plt.legend()
plt.title(f"Distribution of somites for {derivation_name}")
plt.xlabel("Number of somites")
plt.ylabel("Frequency")
plt.text(0, 1.2, r'mean tot   = {:.1f}'.format(np.mean(np.array(n_total))), fontsize=15)
plt.text(0, 1.0, r'median tot = {:.1f}'.format(np.median(np.array(n_total))), fontsize=15)
plt.text(0, 0.8, r'std tot    = {:.1f}'.format(np.std(np.array(n_total))), fontsize=15)

plt.subplot(212)
plt.hist(n_bad, bins=10, alpha=0.6, label="Defective somites", density=True)
plt.xlabel("Number of somites")
plt.ylabel("Frequency")
