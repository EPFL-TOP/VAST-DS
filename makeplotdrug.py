...
plt.subplot(211)
plt.hist(n_total, bins=10, alpha=0.6, label="Total somites and valid fish={1}".format(len(n_total)), density=True)
plt.legend()
...
plt.legend()
plt.title(f"Distribution of somites for {derivation_name} and not valid fish={1}".format(len(n_bad_notv)))
plt.xlabel("Number of somites")
plt.ylabel("Frequency")
