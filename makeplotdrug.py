...
plt.subplot(211)
plt.hist(n_total, bins=10, alpha=0.6, label="Total somites", density=True)
plt.legend()
plt.title(f"Distribution of somites for {derivation_name} and valid fish={len(n_total)}")
...
plt.legend()
plt.title(f"Distribution of somites for {derivation_name} and not valid fish={len(n_bad_notv)}")
plt.xlabel("Number of somites")
plt.ylabel("Frequency")
plt.text(15, 0.5, r'mean tot   = {:.1f}'.format(np.mean(np.array(n_total_notv))), fontsize=15)
plt.text(15, 0.4, r'median tot = {:.1f}'.format(np.median(np.array(n_total_notv))), fontsize=15)
plt.text(15, 0.3, r'std tot    = {:.1f}'.format(np.std(np.array(n_total_notv))), fontsize=15)

plt.subplot(212)
plt.hist(n_bad_notv, bins=20, alpha=0.6, label="Defective somites", density=True)
plt.xlabel("Number of somites")
plt.ylabel("Frequency")
plt.text(0, 0.175, r'mean defective   = {:.1f}'.format(np.mean(np.array(n_bad_notv))), fontsize=15)
plt.text(0, 0.15, r'median defective = {:.1f}'.format(np.median(np.array(n_bad_notv))), fontsize=15)
plt.text(0, 0.125, r'std defective    = {:.1f}'.format(np.std(np.array(n_bad_notv))), fontsize=15)
plt.legend()
plt.show()
